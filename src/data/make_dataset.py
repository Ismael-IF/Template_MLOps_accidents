...
def process_data(input_filepath_users, input_filepath_caract, input_filepath_places, input_filepath_veh, output_filepath):
    ...
    # Add new columns
    df = add_new_columns(df, nb_victim, nb_vehicules) #  commentaire pour une 2e version

    # Modify target variable
    df = modif_target_variable(df)    # version test 

    # Replace values -1 and 0
    df = replace_values(df)       # decommenter cette variable
